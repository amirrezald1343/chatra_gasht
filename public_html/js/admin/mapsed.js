...
(function () {
    ...
    $.fn.mapsed = function (options) {
        ...
        var DEFAULT_CENTER = new google.maps.LatLng(32.79941238968643 , 49.162267056245604);
        var DEFAULT_ZOOM = 4;
        ...
        /// - Builds the model and forwards onto the callback for confirmation.
        /// </summary>
        function onPlaceDelete(element , id) {
        ...
        function buildMsg(title, msg, doConfirm, prompt, callback) {
            ...
            if (doConfirm) {
                buttons += "<button  type='button' class='ok'>بلی</button>";
                buttons += "<button  type='button' class='cancel'>خیر</button>";
            } else {
                buttons += "<button  type='button'  class='close'>بلی</button>";
            }
            buttons += "</div>";
            buttons += "</div>";
        ...
        function attachTooltip(forMarker) {
            ...
            if($('#mname'+ newId ).val()){
                var mname=$('#mname'+ newId ).val();
            }else {
                var mname='';
            }


            if($('#mnumber'+ newId ).val()){
                var mnumber=$('#mnumber'+ newId ).val();
            }else {
                var mnumber=1;
            }
            ...
            var item = $(
                ...
                "<input type='hidden'  name='lat'  id='mapsedlat" + newId + "'  class='mapsed-lat' value='" + forMarker.position.lat() + "' />" +
                "<input type='hidden'  name='lng' id='mapsedlng" + newId + "'  class='mapsed-lng' value='" + forMarker.position.lng() + "' />" +
                ...
                "<ul>" +
                "<li>" +
                "<label> <span class='titformmap'> نام مکان</span> " +
                "<input  value='"+ mname  + "'  type='text'  id='mapsedname" + newId + "'   />" +
                " </label>" +
                "</li>" +
                "<li>" +
                "<label><span class='titformmap'> شماره  مکان</span>   " +
                "<input  value='"+ mnumber +"'   type='number'  min='1'   id='mapsednumber" + newId + "' />" +
                "</label>" +
                "</li>" +
                "</ul>" +
                "<div class='mapsed-buttons'>" +
                "<button  id='"+ newId +"'   onclick='deletplace(" + newId + ")'  type='button'   class='mapsed-delete-button deletplace' style='display: inline-block;'>حذف</button>" +
                "<button   onclick='saveplace(" + newId + ")'  type='button'  class='mapsed-save-button' id='saveplace" + newId + "'  >ذخیره</button>" +
                "<span class='mapsed-error'>&nbsp;</span>" +
                "</div>" +
                "</div>"
            );

            let countOld=document.getElementsByClassName('udatemapOld').length;
            let count=document.getElementsByClassName('udatemap').length+1+countOld;

            var plade=
                      "<div  style='margin: 0 0 9px 0px;' class='udatemap'  id='mapsed" + newId + "' class='mapsed-root'>"
                    + "<span style='color: #2196f3;text-align: center;display: inline-block;width: 70px;margin-left: 9px;border: 1px solid #cbcbcb;padding: 1px 7px 0px 4px;border-radius: 3px;background-color: #fff;'> مکان "+ count +" </span>"
                    + "<input  style='background: #fff;border: 0;border: 1px solid #cbcbcb;border-radius: 3px;color: #777;'   value='" + forMarker.position.lat() + "'  id='mlat" + newId + "'  name='lat[]'  class='mapsed-lat' /> "
                    + "<input style='background: #fff;border: 0;border: 1px solid #cbcbcb;border-radius: 3px;color: #777;'   value='" + forMarker.position.lng() + "'  id='mlng" + newId + "'  name='lng[]' class='mapsed-lng'  /> "
                    + "<input style='background: #fff;border: 0;border: 1px solid #cbcbcb;border-radius: 3px;color: #777;'   id='mname" + newId + "'  name='mapsedname[]'   type='text'  /> "
                    + "<input style='background: #fff;border: 0;border: 1px solid #cbcbcb;border-radius: 3px;color: #777;'   id='mnumber" + newId + "'  name='mapsednumber[]' type='number'  min='1'  value='1'   /> "
                    + "</div>"
                ;
            $('#place').append(plade);
                ...
                _mapContainer.on("click", "button.mapsed-delete-button",
                    function () {
                        ...
                        if (settings.confirmDelete) {
                            var $vw = element.parents(".mapsed-view");
                            var model = getViewModel($vw);
                            var msg = " شما مطمئن هستید؟"
                            _plugIn.confirmMsg("حذف مکان", msg,
                                "",
                                // callback only fired if "Yes" is selected
                                function () {

                                    onPlaceDelete(element , id);
                                }
                            );
                        } else {
                            onPlaceDelete(element , id);
